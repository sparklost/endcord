...
def generate_qr_code_ascii(data: str, border: int = 2) -> str:
    ...
    try:
        ...
        # Use Unicode block characters for 2x1 pixel blocks
        # This makes the QR code more compact vertically
        lines = []
...
def generate_qr_code_simple(data: str) -> str:
    ...
    More compatible with limited terminal fonts.
    """
    try:
        import qrcode
        qr = qrcode.QRCode(
        ...
        for row in matrix:
            line = ""
            for cell in row:
                line += "\u2588\u2588" if cell else "  "  # Double-width for square pixels
            lines.append(line)

        return "\n".join(lines)

    except ImportError:
        return f"[Install 'qrcode' package for QR display]\nURL: {data}"
